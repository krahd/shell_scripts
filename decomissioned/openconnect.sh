...
VPN_USERNAME="tola1460#VPN_TUNNEL"
VPN_GROUP=""
...
# No storing passwords unenin plain text fiTsecurity find-generic-password -wl $VPN_HOST"
...
case "$1" in
    connect)
        ...
        # --external-browser="/System/Volumes/Preboot/Cryptexes/App/System/Applications/Safari.app/Contents/MacOS/Safari"
        echo -e "${VPN_PASSWORD}\n$(prompt_2fa_method ${PUSH_OR_PIN})\n" | sudo "$VPN_EXECUTABLE" --useragent=AnyConnect --passwd-on-stdin -u "$VPN_USERNAME" -i "$VPN_INTERFACE" "$VPN_HOST/$VPN_GROUP" & #&> /dev/null &
